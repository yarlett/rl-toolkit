...
pub trait Domain {
    ...
    fn act(&mut self, _: Self::Agent, _: Self::Action) -> Vec<(Self::Agent, Self::Reward)>;
    ...
    fn simulate(
        ...
        loop {
            ...
            let (agent, action) = actions.choose(&mut rand::thread_rng()).unwrap();
            let rewards = self.act(agent.to_owned(), action.to_owned());
            data.push((state, agent.to_owned(), action.to_owned(), rewards));
        }
